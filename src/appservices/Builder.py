...
class AppServicesBuilder:
    ...
    def __init__(self, **kwargs):
        ...
        self.options["preso_fn"] = os.path.join(
            self.options["workingdir"],
            self.options['resource_dir'],
            self.options["preso"])

        self.options["impl_fn"] = os.path.join(
            self.options["workingdir"],
            self.options['resource_dir'],
            self.options["impl"])
        ...
        self._load_info()
        self.buildinfo['template_name'] = "appsvcs_integration_v{}.{}{}".\
            format(self.buildinfo["impl_major"],
                   self.buildinfo["impl_minor"],
                   self.options["append"])

        if not self.options["outfile"]:
            self.options['outfile_fn'] = os.path.join(
                self.options['workingdir'],
                self.options['build_dir'],
                "{}.tmpl".format(self.buildinfo['template_name'])
            )
        else:
            self.options["outfile_fn"] = os.path.join(
                self.options["workingdir"],
    ...
    def _load_info(self):
        ...
        for line in impl:
            ...
            if implmajormatch:
                self.buildinfo["impl_major"] = implmajormatch.group(1)

            if implminormatch:
                self.buildinfo["impl_minor"] = implminormatch.group(1)

            if self.buildinfo["impl_major"] and self.buildinfo["impl_minor"]:
                break
        impl.close()
        if self.pres_data["tmpl_majorversion"] != self.buildinfo["impl_major"]:
            print("[fatal] IMPLVERSION_MAJOR ({}) in {}"
                  " does not match 'tmpl_majorversion' ({}) in {}".format(
        ...
        for section in self.pres_data["sections"]:
            ...
            for field in section["fields"]:
                self.buildinfo["allvars"].append("{}__{}".format(
                    section["name"], field["name"]))

        self.buildinfo["allvarsTCL"] = "\n".join(
            map(' {0} \\\\'.format, self.buildinfo["allvars"]))

        ver = self._safe_open(
            os.path.join(self.options["workingdir"], 'VERSION'))
    ...
    def _safe_open(self, filename, mode="r", exit=True):
        ...
        try:
            ...
            print("[fatal] Open of file '{}' failed: {}".format(
                filename, error))
            print('-'*60)
    ...
    def _tmpl_process_file(self, fin, fout, prepend=""):
        ...
        for line in fin:
            ...
            if match:
                insertfile = self._safe_open(os.path.join(
                    self.options["resource_dir"],
                    match.group(2)))
    ...
    def _apl_generate_field(self, field, section, tab):
        ...
        if hasattr(AppServicesBuilder, func_name):
            apl_field = getattr(AppServicesBuilder, func_name)(self, field, tab)
        else:
    ...
    def _iwfjson_generate_field(self, field, section=None):
        ...
        if 'choices' in field.keys() and \
            ...
            tempChoices = map(lambda x: x.replace('&lt;', '<'),
                              field["choices"])
            tempChoices = map(lambda x: x.replace('&gt;', '>'),
                              tempChoices)
            ret['choices'] = map(lambda x: {
                'value': x, 'description': x}, tempChoices)
    ...
    def createBundledResources(self):
        ...
        for filename in files:
            ...
            if re.match(r'.*irules.*irule$', filename):
                filetype = 'irule'
            if re.match(r'.*asm.*xml$', filename):
                filetype = 'asm'
            if re.match(r'.*apm.*.tar.gz$', filename):
                filetype = 'apm'
    ...
    def buildAPL(self, **kwargs):
        ...
        fh = self._safe_open(
            os.path.join(
                self.options["workingdir"],
                self.options["tempdir"], 'apl.build'), "wt")
    ...
    def buildiWfTemplate(self, **kwargs):
        ...
        fh = self._safe_open(os.path.join(
            self.options["workingdir"],
            self.options['build_dir'],
            "iWorkflow_{}.json".format(self.buildinfo['template_name'])), "wt")
    ...
    def buildJsonTemplate(self, **kwargs):
        ...
        fh = self._safe_open(os.path.join(
            self.options["workingdir"],
            self.options['build_dir'],
            "BIGIP_{}.json".format(self.buildinfo['template_name'])), "wt")
    ...
    def buildTemplate(self, **kwargs):
        ...
        main = self._safe_open(
            os.path.join(
                self.options["resource_dir"],
                self.options["roottmpl"]))
