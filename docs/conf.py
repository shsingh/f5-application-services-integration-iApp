...
options["outfile"] = os.path.join(
    'docs', '_static', 'appsvcs_integration_v%s.tmpl' % releasefn)

if on_rtd:
    os.mkdir("../rtdtemp")
    builder.buildAPL(build_dir=build_dir,
                     resource_dir=resource_dir,
                     tempdir=tmp_dir,
                     bundledir=bundle_dir)
    ...
    options["outfile"] = os.path.join(
        'docs', '_static', 'appsvcs_integration_v%s.tcl' % releasefn)
    options["roottmpl"] = os.path.join(
        'src', 'implementation_only.template')
    builder.buildTemplate(**options)

    # Build the APL only file
    options["outfile"] = os.path.join(
        'docs', '_static', 'appsvcs_integration_v%s.apl' % releasefn)
    options["roottmpl"] = os.path.join(
        'rtdtemp', 'apl.build')
