...
function f5_check_response(vars, test) {
    ...
    for (var i = 0; i < vars.length; i++) {
        if (typeof vars[i].path === 'undefined' ||
            ...
            console.log('f5_check_response: ERROR: element ' + i + ' does not have path and value attributes ' + JSON.stringify(vars[i]));
            continue;
        }
...
function f5_check_response_code(mode) {
    ...
    if (responseCode.code === 400) {
        var json = f5_parse_json_resp();
        f5_set_test_result("Error message: " + json.message, 0);
    }
...
function f5_set_test_result(name, result, value, polloverride) {
    ...
    polloverride = typeof polloverride  !== 'undefined';
