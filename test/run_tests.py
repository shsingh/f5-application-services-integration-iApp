...
def run_test():
    ...
    version = get_version(args.username, args.password, args.host)
    ...
    if args.SSHPort != 22:
        exitcode, out, err = get_exitcode_stdout_stderr(
            "scp -P {} test_config.conf root@{}:/var/tmp".format(args.SSHPort, host))
    else:
    ...
    if args.SSHPort != 22:
        exitcode, out, err = get_exitcode_stdout_stderr(
            "ssh -p {} root@{} 'tmsh -c \"delete ltm pool all; delete ltm node all\"'".format(args.SSHPort, host))
    else:
        exitcode, out, err = get_exitcode_stdout_stderr(
            "ssh root@%s 'tmsh -c \"delete ltm pool all; delete ltm node all\"'" % args.host)
    print out
    print err
    # dirty hack for ci-integration
    if args.SSHPort != 22:
        exitcode, out, err = get_exitcode_stdout_stderr(
            "ssh -p {} root@{} 'tmsh load sys config file /var/tmp/test_config.conf merge'".format(args.SSHPort, host))
    else:
        exitcode, out, err = get_exitcode_stdout_stderr(
            "ssh root@%s 'tmsh load sys config file /var/tmp/test_config.conf merge'" % args.host)
    print out
    print err
    ...
    for test_template in test_templates:
        ...
        for i in range(args.retries):
            ...
            if not exitcode:
                ...
                # dirty hack for ci-integration
                if args.SSHPort != 22:
                    resultcmd = "ssh -p {} root@{} \"tmsh -c 'cd /{}/{}.app ; list ltm ; list asm ; list apm'\"".format(
                        args.SSHPort, host, del_partition, res_name)
                else:
...
def get_version(username, password, host):
    s = requests.session()
    s.auth = (username, password)
    s.verify = False
    resp = s.get("https://%s/mgmt/tm/sys/software/volume?$select=active,version" % host)

    if resp.status_code == 401:
        print "[error] Authentication to %s failed" % host
        sys.exit(1)
...
parser.add_argument("-ssh", "--SSHPort", help="Non default SSH port", type=int, default=22)
